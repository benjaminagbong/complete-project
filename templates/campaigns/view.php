<?php 
$mail_subject	= '&quot;'.$view_data->b_f_name.' '.$view_data->b_l_name.'&quot; needs your urgent help';
$mail_body		= 'Hello&#44; %2C%0A%0AI wanted to let you know that &quot;'.$view_data->b_f_name.' '.$view_data->b_l_name.'&quot; currently needs help. &quot;'.$view_data->b_f_name.' '.$view_data->b_l_name.'&quot; and his family will be so grateful if you could help or share this on social media . Here is a link to his situation ';
?>
<?php
...
.causes-details-image .causes-main-image{
	width:100%;
}
.causes-verified-img img{
    position: absolute;
    width: 100px;
    right: 10px;
    top: 10px;
}
.report-fundraiser-link a{
...
<?php
if($view_data->is_verified==1){
?>
                    	<div class="causes-verified-img"><img src="assets/themes/img/icon/verified.png" /></div>
<?php
}
?>
<img src="<?=site_url().'ajax_image/path?src='.$view_data->image?>" alt="image" class="causes-main-image">
</div>
...
<div class="status">
    <div class="status-title">Raised:		<?=currency_symbol($view_data->currency)?><?=$campaign_payment?></div>
    <div class="status-number">         <span>Goal:</span> 
		<?=currency_symbol($view_data->currency)?><?=$view_data->campaign_funds?>
 </div>
</div>
</div>
...
?>
    ...
    <div class="col-md-12 report-fundraiser-link"><i class="fa fa-flag"></i> <a href="reports_campaign/l/<?=$view_data->id?>">Report Fundraiser</a></div>
...
?>
<a href="<?=site_url().'paypal_payment/form/'.$view_data->id?>" class="default-btn mt-2">Donate With Paypal</a>
<?php
}
if(print_count('payment_setting',array('id'=>1,'enabled'=>1))>0){
?>
<a href="<?=site_url().'stripe_payment/form/'.$view_data->id?>" class="default-btn mt-2">Donate With Debit/Credit card</a>
<?php
}
if(print_count('payment_setting',array('id'=>3,'enabled'=>1))>0){
?>
<a href="<?=site_url().'paystack_payment/form/'.$view_data->id?>" class="default-btn mt-2">Donate With Paystack</a>
<?php
}
if(print_count('payment_setting',array('id'=>4,'enabled'=>1))>0){
?>
<a href="<?=site_url($_cancel).'/bank_payment/'.$view_data->id?>" class="default-btn mt-2">Donate With Bank</a>
<?php
}
}
elseif($this->data['session_data']['license_session']['cause_count']==30){
if(print_count('payment_setting',array('id'=>2,'enabled'=>1))>0){
?>
<a href="<?=site_url().'paypal_payment/form/'.$view_data->id?>" class="default-btn mt-2">Donate With Paypal</a>
<?php	
...
elseif($this->data['session_data']['license_session']['cause_count']==70){
if(print_count('payment_setting',array('id'=>2,'enabled'=>1))>0){
?>
<a href="<?=site_url().'paypal_payment/form/'.$view_data->id?>" class="default-btn mt-2">Donate With Paypal</a>
<?php
...
<li class="m-share-list-item"><a class="cause-share-btn" href="mailto:?subject=<?=$mail_subject?>?&body=<?=$mail_body?>,
<?=site_url(uri_string());?>. 
" >
...
<li class="m-share-list-item">
<a class="cause-share-btn" href="https://api.whatsapp.com/send?text=Hello%2C%0A%0AI wanted to let you know that <?=$view_data->b_f_name.' '.$view_data->b_l_name?> currently needs help. <?=$view_data->b_f_name.' '.$view_data->b_l_name?> and his family will be so grateful if you could help or share this on social media . Here is a link to his situation %0A%0A<?=site_url(uri_string());?>%0A%0AWe really appreciate and thanks for sharing." data-action="share/whatsapp/share"><i class="fa fa-whatsapp"></i>
WhatsApp</a>
</li>
